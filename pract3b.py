#A program to implement hill climbing problem 
...
def newDistance (x1, y1, point1, point2, point3, point4):
    d1 = [x1, y1]
    d1_temp = sumOfDistances (x1, y1, point1[0], point1[1],
...
Flag = True
def newPoint (minimum, d1, d2, d3, d4):
    if d1[2] == minimum:
        return [d1[0], d1[1]]
    elif d2[2] == minimum:
        return [d2[0], d2[1]]
...
while Flag:
    ...
    print(f"i={i} round ({startingPoint[0]}, {startingPoint[1]}")
    print (d1[2], d2[2], d3[2], d4[2])
    minimum = min(d1[2], d2[2], d3[2], d4[2])
    
    if minimum < minDistance:
        startingPoint = newPoint(minimum, d1, d2, d3, d4)
        minDistance = minimum
        i += 1
    else:
        Flag = False